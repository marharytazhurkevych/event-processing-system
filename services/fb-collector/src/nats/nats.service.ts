...
export class NatsService implements OnModuleDestroy {
  ...
  async subscribeToFacebookEvents(
    ...
    try {
      ...
      const consumer = await (this.jetStream.consumers as any).add('FACEBOOK_EVENTS', consumerConfig);
      ...
      for await (const message of messages) {
        ...
        try {
          ...
          this.logger.log(`Processed Facebook event in ${processingTime}ms`);
          message.ack();
          
        } catch (error) {
          const processingTime = Date.now() - startTime;
          this.logger.error(`Failed to process Facebook event after ${processingTime}ms: ${error.message}`, error.stack);
